...
public:
	...
	{
		JPH_ASSERT(inLeftRatio != 0.0f && inRightRatio != 0.0f);
		mForwardInput = inForward;
		mLeftRatio = inLeftRatio;
		mRightRatio = inRightRatio;
		mBrakeInput = inBrake;
	}
	...
	{
		mRPMMeterPosition = inPosition;
		mRPMMeterSize = inSize;
	}
...
protected:
	...
	{
		JPH_ASSERT(IsKindOf(&inWheel, JPH_RTTI(WheelSettingsTV)));
		return new WheelTV(static_cast<const WheelSettingsTV &>(inWheel));
	}
