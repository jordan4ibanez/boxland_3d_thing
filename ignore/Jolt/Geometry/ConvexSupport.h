...
struct TransformedConvexObject
{
	...
	TransformedConvexObject(Mat44Arg inTransform, const ConvexObject &inObject) : mTransform(inTransform),
																																								mObject(inObject)
	{
	}
...
{
	AddConvexRadius(const ConvexObject &inObject, float inRadius) : mObject(inObject),
																																	mRadius(inRadius)
	{
	}
...
{
	MinkowskiDifference(const ConvexObjectA &inObjectA, const ConvexObjectB &inObjectB) : mObjectA(inObjectA),
																																												mObjectB(inObjectB)
	{
	}
...
{
	/// Constructor
	TriangleConvexSupport(Vec3Arg inV1, Vec3Arg inV2, Vec3Arg inV3) : mV1(inV1),
																																		mV2(inV2),
																																		mV3(inV3)
	{
	}
...
{
	/// Constructor
	explicit PolygonConvexSupport(const VERTEX_ARRAY &inVertices) : mVertices(inVertices)
	{
	}
