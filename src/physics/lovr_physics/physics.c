...
#define vec3_toJolt(v) \
  &(JPH_Vec3) { v[0], v[1], v[2] }
#define vec3_fromJolt(v, j) vec3_set(v, (j)->x, (j)->y, (j)->z)
#define quat_toJolt(q) \
  &(JPH_Quat) { q[0], q[1], q[2], q[3] }
...
static JPH_BroadPhaseLayerFilter *getBroadPhaseLayerFilter(World *world, uint32_t filter)
{
  ...
  if (~world->staticTagMask & filter)
    thread.broadPhaseLayerMask |= 0x1;
  if (world->staticTagMask & filter)
    thread.broadPhaseLayerMask |= 0x3;
...
static JPH_ValidateResult onContactValidate(void *userdata, const JPH_Body *body1, const JPH_Body *body2, const JPH_RVec3 *offset, const JPH_CollideShapeResult *result)
{
  ...
  thread.locked = false;
  mtx_unlock(&world->lock);
  return accept ? JPH_ValidateResult_AcceptAllContactsForThisBodyPair : JPH_ValidateResult_RejectAllContactsForThisBodyPair;
}
...
World *lovrWorldCreate(WorldInfo *info)
{
  ...
  world->bodyInterfaceLocked = info->threadSafe ? JPH_PhysicsSystem_GetBodyInterface(world->system) : world->bodyInterfaceNoLock;
...
void lovrColliderDestruct(Collider *collider)
{
  ...
  if (world->colliders == collider)
    world->colliders = collider->next;
...
bool lovrColliderSetFriction(Collider *collider, float friction)
{
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_SetFriction(interface, collider->id, MAX(friction, 0.f));
  return !!interface;
}
...
bool lovrColliderSetRestitution(Collider *collider, float restitution)
{
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_SetRestitution(interface, collider->id, MAX(restitution, 0.f));
  return !!interface;
}
...
bool lovrColliderSetContinuous(Collider *collider, bool continuous)
{
  ...
  if (interface)
    JPH_BodyInterface_SetMotionQuality(interface, collider->id, quality);
  return !!interface;
}
...
bool lovrColliderSetGravityScale(Collider *collider, float scale)
{
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_SetGravityFactor(interface, collider->id, scale);
  return !!interface;
}
...
void lovrColliderSetDegreesOfFreedom(Collider *collider, bool translation[3], bool rotation[3])
{
  ...
  if (translation[0])
    dofs |= JPH_AllowedDOFs_TranslationX;
  if (translation[1])
    dofs |= JPH_AllowedDOFs_TranslationY;
  if (translation[2])
    dofs |= JPH_AllowedDOFs_TranslationZ;
  if (rotation[0])
    dofs |= JPH_AllowedDOFs_RotationX;
  if (rotation[1])
    dofs |= JPH_AllowedDOFs_RotationY;
  if (rotation[2])
    dofs |= JPH_AllowedDOFs_RotationZ;
...
bool lovrColliderMoveKinematic(Collider *collider, float position[3], float orientation[4], float dt)
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_MoveKinematic(interface, collider->id, vec3_toJolt(position), quat_toJolt(orientation), dt);
  return !!interface;
}
...
bool lovrColliderSetLinearVelocity(Collider *collider, float velocity[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_SetLinearVelocity(interface, collider->id, vec3_toJolt(velocity));
  return !!interface;
}
...
bool lovrColliderSetAngularVelocity(Collider *collider, float velocity[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_SetAngularVelocity(interface, collider->id, vec3_toJolt(velocity));
  return !!interface;
}
...
bool lovrColliderApplyForce(Collider *collider, float force[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_AddForce(interface, collider->id, vec3_toJolt(force));
  return !!interface;
}
...
bool lovrColliderApplyForceAtPosition(Collider *collider, float force[3], float position[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_AddForce2(interface, collider->id, vec3_toJolt(force), vec3_toJolt(position));
  return !!interface;
}
...
bool lovrColliderApplyTorque(Collider *collider, float torque[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_AddTorque(interface, collider->id, vec3_toJolt(torque));
  return !!interface;
}
...
bool lovrColliderApplyLinearImpulse(Collider *collider, float impulse[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_AddImpulse(interface, collider->id, vec3_toJolt(impulse));
  return !!interface;
}
...
bool lovrColliderApplyLinearImpulseAtPosition(Collider *collider, float impulse[3], float position[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_AddImpulse2(interface, collider->id, vec3_toJolt(impulse), vec3_toJolt(position));
  return !!interface;
}
...
bool lovrColliderApplyAngularImpulse(Collider *collider, float impulse[3])
{
  ...
  JPH_BodyInterface *interface = getBodyInterface(collider, WRITE);
  if (interface)
    JPH_BodyInterface_AddAngularImpulse(interface, collider->id, vec3_toJolt(impulse));
  return !!interface;
}
...
void lovrShapeGetPose(Shape *shape, float position[3], float orientation[4])
{
  if (shape->collider)
  {
    ...
    if (orientation)
      quat_mul(orientation, colliderOrientation, shape->rotation);
  }
  else
  {
    if (position)
      vec3_init(position, shape->translation);
    if (orientation)
      quat_init(orientation, shape->rotation);
  }
}
...
void lovrJointDestruct(Joint *joint)
{
  ...
  if (a)
  {
    node = &joint->a;
    if (node->next)
      lovrJointGetNode(node->next, a)->prev = node->prev;
    if (node->prev)
      lovrJointGetNode(node->prev, a)->next = node->next;
    else
      a->joints = node->next;
  }
  ...
  node = &joint->b;
  if (node->next)
    lovrJointGetNode(node->next, b)->prev = node->prev;
  if (node->prev)
    lovrJointGetNode(node->prev, b)->next = node->next;
  else
    b->joints = node->next;

  node = &joint->world;
  if (node->next)
    node->next->world.prev = node->prev;
  if (node->prev)
    node->prev->world.next = node->next;
  else
    world->joints = joint->world.next;
...
float lovrHingeJointGetMotorTarget(HingeJoint *joint)
{
  ...
  return motorState == JPH_MotorState_Position ? JPH_HingeConstraint_GetTargetAngle(constraint) : JPH_HingeConstraint_GetTargetAngularVelocity(constraint);
}
...
float lovrSliderJointGetMotorTarget(SliderJoint *joint)
{
  ...
  return motorState == JPH_MotorState_Position ? JPH_SliderConstraint_GetTargetPosition(constraint) : JPH_SliderConstraint_GetTargetVelocity(constraint);
}
