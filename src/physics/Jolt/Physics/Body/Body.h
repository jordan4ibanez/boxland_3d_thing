...
#include "../Body/BodyAccess.h"
#include "../Body/BodyType.h"
...
public:
	...
	inline void SetCollideKinematicVsNonDynamic(bool inCollide)
	{
		JPH_ASSERT(IsRigidBody());
		if (inCollide)
			mFlags.fetch_or(uint8(EFlags::CollideKinematicVsNonDynamic), memory_order_relaxed);
		else
			mFlags.fetch_and(uint8(~uint8(EFlags::CollideKinematicVsNonDynamic)), memory_order_relaxed);
	}
	...
	inline void SetUseManifoldReduction(bool inUseReduction)
	{
		JPH_ASSERT(IsRigidBody());
		if (inUseReduction)
			mFlags.fetch_or(uint8(EFlags::UseManifoldReduction), memory_order_relaxed);
		else
			mFlags.fetch_and(uint8(~uint8(EFlags::UseManifoldReduction)), memory_order_relaxed);
	}
	...
	inline void SetApplyGyroscopicForce(bool inApply)
	{
		JPH_ASSERT(IsRigidBody());
		if (inApply)
			mFlags.fetch_or(uint8(EFlags::ApplyGyroscopicForce), memory_order_relaxed);
		else
			mFlags.fetch_and(uint8(~uint8(EFlags::ApplyGyroscopicForce)), memory_order_relaxed);
	}
	...
	inline void SetEnhancedInternalEdgeRemoval(bool inApply)
	{
		JPH_ASSERT(IsRigidBody());
		if (inApply)
			mFlags.fetch_or(uint8(EFlags::EnhancedInternalEdgeRemoval), memory_order_relaxed);
		else
			mFlags.fetch_and(uint8(~uint8(EFlags::EnhancedInternalEdgeRemoval)), memory_order_relaxed);
	}
	...
	inline Vec3 GetPointVelocity(RVec3Arg inPoint) const
	{
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read));
		return GetPointVelocityCOM(Vec3(inPoint - mPosition));
	}
	...
	{
		JPH_ASSERT(IsDynamic());
		return mMotionProperties->GetAccumulatedForce();
	}
	...
	{
		JPH_ASSERT(IsDynamic());
		return mMotionProperties->GetAccumulatedTorque();
	}
	...
	{
		JPH_ASSERT(IsDynamic());
		return mMotionProperties->ResetForce();
	}
	...
	{
		JPH_ASSERT(IsDynamic());
		return mMotionProperties->ResetTorque();
	}
	...
	{
		JPH_ASSERT(!IsStatic());
		return mMotionProperties->ResetMotion();
	}
	...
	inline RVec3 GetPosition() const
	{
		JPH_ASSERT(BodyAccess::sCheckRights(BodyAccess::sPositionAccess(), BodyAccess::EAccess::Read));
	...
	inline void AddPositionStep(Vec3Arg inLinearVelocityTimesDeltaTime)
	{
		...
		mPosition += mMotionProperties->LockTranslation(inLinearVelocityTimesDeltaTime);
	...
	{
		JPH_IF_ENABLE_ASSERTS(uint8 old_val =)
		mFlags.fetch_and(uint8(~uint8(EFlags::InvalidateContactCache)), memory_order_relaxed);
