...
bool OrientedBox::Overlaps(const AABox &inBox, float inEpsilon) const
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		rb = mHalfExtents[i];
		if (abs(rot.GetTranslation().Dot(rot.GetColumn3(i))) > ra + rb)
			return false;
	}
